...
static void test_https_proxy(void)
{
    ...
    g_assert( pb_config_set_https_proxy(c, "localhost") == 0 );
    g_assert( pb_config_set_https_proxy(d, "localhost") == -1 );
...
static void test_timeout(void)
{
    ...
    g_assert( pb_config_get_timeout(c) == 0 );
    g_assert( pb_config_get_timeout(d) == 0 );

    g_assert( pb_config_set_timeout(c, 5) == 0 );
    g_assert( pb_config_set_timeout(d, 0) == -1 );

    g_assert( pb_config_get_timeout(c) == 5 );
    g_assert( pb_config_get_timeout(d) == 0 );
...
static void test_token_key(void)
{
    ...
    g_assert( pb_config_set_token_key(c, "hello_world") == 0 );
    g_assert( pb_config_set_token_key(d, "hello_world") == -1 );
...
static void test_from_json_file(void)
{
    ...
    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, NULL) == -1);
    pb_config_unref(c);

    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/") == -1);
    pb_config_unref(c);

    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/null.json") == -1);
    pb_config_unref(c);

    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/empty.json") == -1);
    pb_config_unref(c);

    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/empty_json.json") == 0);
    g_assert( pb_config_get_token_key(c) == NULL );
    g_assert( pb_config_get_https_proxy(c) == NULL );
    g_assert( pb_config_get_timeout(c) == 0 );
    pb_config_unref(c);
    ...
    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/empty_values.json") == 0);
    g_assert( pb_config_get_token_key(c) != NULL );
    g_assert( pb_config_get_https_proxy(c) != NULL );
    g_assert( strlen(pb_config_get_token_key(c)) == 0 );
    g_assert( strlen(pb_config_get_https_proxy(c)) == 0 );
    g_assert( pb_config_get_timeout(c) == 0 );
    pb_config_unref(c);

    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/proxies.json") == 0);
    g_assert( pb_config_get_token_key(c) == NULL );
    g_assert( pb_config_get_https_proxy(c) != NULL );
    g_assert( strlen(pb_config_get_https_proxy(c)) != 0 );
    g_assert_cmpstr( pb_config_get_https_proxy(c), ==, "localhost" );
    ...
    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/timeout.json") == 0);
    g_assert( pb_config_get_token_key(c) == NULL );
    g_assert( pb_config_get_https_proxy(c) == NULL );
    g_assert( pb_config_get_timeout(c) ==  2 );
    pb_config_unref(c);

    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/token_key.json") == 0);
    g_assert( pb_config_get_https_proxy(c) == NULL );
    g_assert( pb_config_get_token_key(c) != NULL );
    g_assert( strlen(pb_config_get_token_key(c)) != 0 );
    ...
    c = pb_config_new();
    g_assert( c != NULL );
    g_assert(pb_config_from_json_file(c, "conf/all.json") == 0);
    g_assert( pb_config_get_https_proxy(c) != NULL );
    g_assert( strlen(pb_config_get_https_proxy(c)) != 0 );
    g_assert_cmpstr( pb_config_get_https_proxy(c), ==, "localhost" );
    g_assert( pb_config_get_token_key(c) != NULL );
